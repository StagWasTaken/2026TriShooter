...
public class HoodConfig {
  ...
  static {
    if (Robot.CURRENT_ROBOT == RobotName.COMP_BOT) {
      hoodConfig
          .disableFollowerMode()
      ...
      hoodConfig
          ...
          .inverted(false)
          .positionConversionFactor((2 * Math.PI) / HoodConstants.kGearRatio)
          .velocityConversionFactor((2 * Math.PI) / (60 * HoodConstants.kGearRatio));
      hoodConfig
          .closedLoop
          .pid(HoodConstants.kP, HoodConstants.kI, HoodConstants.kD, ClosedLoopSlot.kSlot0)
          .outputRange(HoodConstants.kMinOutput, HoodConstants.kMaxOutput)
